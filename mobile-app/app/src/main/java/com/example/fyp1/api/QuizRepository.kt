...
class QuizRepository(context: Context) {
    ...
    private val api: QuizApiService by lazy {
    ...
    suspend fun getQuizForContent(contentId: String): AuthResult<BackendQuiz> =
        runCatching {
            val quizzes = handleResponse(api.getQuizzes(contentId)) { it.data.quizzes }
            if (quizzes is AuthResult.Error) return quizzes

            val quiz = (quizzes as AuthResult.Success).value.firstOrNull()
                ?: return AuthResult.Error("No quiz is available for this content yet.")

            handleResponse(api.getQuizById(quiz.id)) { it.data.quiz }
        }.getOrElse { AuthResult.Error(networkErrorMessage(it)) }

    suspend fun getMyProgressForContent(contentId: String): AuthResult<BackendLearningProgress> =
        runCatching {
            handleResponse(api.getMyProgressForContent(contentId)) { it.data.progress }
        }.getOrElse { AuthResult.Error(networkErrorMessage(it)) }
    ...
    ): AuthResult<QuizAttemptData> =
        runCatching {
            handleResponse(api.submitAttempt(quizId, SubmitQuizAttemptRequest(answers, timeSpentSeconds))) { it.data }
        }.getOrElse { AuthResult.Error(networkErrorMessage(it)) }
    ...
    private fun parseError(response: Response<*>): String {
        val fallback = when (response.code()) {
            400 -> "Quiz is not ready yet."
            401 -> "Please log in again."
            404 -> "Quiz not found."
            in 500..599 -> "Server error. Please try again later."
            else -> "Could not load quiz."
        }
