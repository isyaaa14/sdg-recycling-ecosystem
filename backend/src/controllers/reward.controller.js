...
export async function completeRedemptionHandler(request, response) {
  try {
    const redemption = await completeRedemption(request.params.id, request.user.id);
    return response.json({ data: { redemption } });
  } catch (error) {
    return handleError(error, response);
  }
...
export async function cancelRedemptionHandler(request, response) {
  try {
    const redemption = await cancelRedemption(request.params.id, request.body, request.user);
    return response.json({ data: { redemption } });
  } catch (error) {
    return handleError(error, response);
  }
